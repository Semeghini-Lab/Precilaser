...
Each card shows protection (PD/T), stage currents, photodiodes, temperatures, and controls (enable/disable drivers, actual current, target current). Enter a target current and press **Enter** to ramp.
...
- Blocked unless **all three drivers are enabled** and **no PD/T protection faults**
- Re-checked **before every step**; ramp aborts if drivers drop or a protection fault appears
...
- If the ramp-to-zero fails (protection/driver fault), drivers stay enabled
...
- Protection, PD trips, and stage colors show faults from live status (not button state)
